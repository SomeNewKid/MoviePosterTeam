...
def _handle_message_send(request: dict[str, object]) -> dict[str, object]:
    ...
    task_id = f"artist-task-{uuid.uuid4()}"
    context_id = f"artist-context-{uuid.uuid4()}"
    task = build_task(
        task_id,
        context_id,
        "TASK_STATE_SUBMITTED",
        message_text="Artist illustration task submitted.",
    )
    _set_task(task_id, task)

    thread = threading.Thread(
        target=_run_artist_task,
        args=(task_id, context_id, movie_details or _DEFAULT_MOVIE_DETAILS),
        daemon=True,
    )
    thread.start()
...
def _run_artist_task(task_id: str, context_id: str, movie_details: str) -> None:
    _set_task(
        task_id,
        build_task(
            task_id,
            context_id,
            "TASK_STATE_WORKING",
            message_text="Artist illustration task is running.",
        ),
    )
    try:
        illustration = run_artist_agent(movie_details)
    except Exception as error:
        _set_task(
            task_id,
            build_task(
    ...
    _set_task(
        ...
        build_task(
            ...
            message_text="Artist illustration task completed.",
            artifacts=(artifact,),
        ),
    )
